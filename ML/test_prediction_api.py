...
class MHUncertaintyEstimator:
    ...
    def estimate_sigma(self, errors_window: np.ndarray, initial_sigma: float) -> float:
        ...
        seed_val = int(np.sum(np.abs(errors_window)) * 10000) % (2**32)
        ...
        for _ in range(self.iterations):
            proposed_sigma = rng.normal(current_sigma, self.step_size)
            if proposed_sigma <= 0.001: continue
            proposed_ll = log_likelihood(proposed_sigma)
            if proposed_ll > current_ll or rng.rand() < np.exp(proposed_ll - current_ll):
...
def predict_manual(sensor: SensorInput):
    ...
    live_window.loc[idx, 'Timestamp'] = pd.Timestamp.now()
