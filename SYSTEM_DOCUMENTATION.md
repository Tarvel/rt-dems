...
| **ML Team** | Machine learning model that predicts energy usage | Predictions published to MQTT | Nothing — they just publish |
| **Us (Backend/Edge)** | The Raspberry Pi server, database, MQTT broker, API endpoints, relay control logic | REST API endpoints, MQTT topics for live data | We receive sensor + ML data via MQTT |
...
|--------------|-------------------|--------|
| ≥ 80% | No (stable) | **Mode A** — Full power, everything on |
...
**Why this re-routing exists:** When energy supply is low but battery is high (≥60%), we can run at full power by drawing from the battery. But if both energy supply is low AND battery is low, we cannot take that risk, so we fall back to the conservative Step 6 rules.
