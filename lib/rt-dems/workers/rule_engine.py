...
EVAL_INTERVAL = int(os.environ.get("RULE_EVAL_INTERVAL_SECONDS", 5))

# Number of readings used for battery stability lock.
BATTERY_STABILITY_READINGS = int(
    os.environ.get("BATTERY_STABILITY_READINGS", 36) # 3 minutes at 5s intervals
)
...
def run_evaluation(client: mqtt.Client) -> None:
    ...
    relay_payload = {
        ...
        "relay_3": r3,
        "battery_lag_values": [
            round(battery_window[-1], 1) if len(battery_window) > 0 else 0,
            round(battery_window[-len(battery_window)//2], 1) if len(battery_window) > 1 else 0,
            round(battery_window[0], 1) if len(battery_window) > 2 else 0,
        ],
        "battery_lag_interval_seconds": EVAL_INTERVAL * (len(battery_window) // 2),
        "reason": reason,
