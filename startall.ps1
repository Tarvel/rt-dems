...
try {
    ...
    Write-Host "-> Starting FastAPI ML Service (new_ml_model)..."
    $mlDir = Join-Path $BASE_DIR "new_ml_model"
    Start-TrackedProcess -FilePath $VENV_PYTHON -ArgumentList "new_prediction_api.py" -WorkingDirectory $mlDir
